...
2. **Optimized Résumé** — the complete résumé in clean, copy-ready markdown, in the MCS template layout: name centered; one contact line (`City, State • email • phone • links`); fixed section order — an optional Summary (only for senior, technical-lead, management, or career-changer profiles; placed directly below the contact line) → Education → Experience → Projects (only when the candidate has relevant projects) → Leadership & Activities (only when such items exist) → Awards & Publications (only when such items exist) → Skills & Interests; each Education, Experience, Projects, and Leadership entry headed by two lines — organization, institution, or project name left with city, state (or "remote") or date context right, then degree, title, or role left with `Month Year – Month Year` right — followed by action-verb bullets without personal pronouns (bullets are required for Experience entries and optional for the others, which may instead carry a single note line); an optional Education note line may hold GPA, coursework, or honors; Awards & Publications as concise bullet lines; Skills & Interests as labeled lines (`Technical: …`, `Language: …`, `Interests: …` — only labels that have content). **Include every work experience with at least its header and one bullet; omit an experience only when it is clearly irrelevant to the target job (a high bar — when in doubt, include it) and name every omission in Changes Made.** Only supported information. Use visible placeholders like "[metric needed]" only when the user explicitly asked for a template; otherwise write the strongest accurate bullet without the missing metric.
